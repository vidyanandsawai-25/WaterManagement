...
import { LayoutDashboard, CheckCircle, ChevronLeft, ChevronRight, Sparkles, X } from 'lucide-react';
...
export function Sidebar({ activeTab, onTabChange, isMobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  const sidebarContent = (
    <motion.div
      ...
      onMouseLeave={() => !window.matchMedia('(max-width: 1024px)').matches && setIsCollapsed(true)}
      role="navigation"
    ...
    >
      ...
      {isMobileOpen && onMobileClose && (
        <button
          ...
          className="lg:hidden absolute top-4 right-4 z-50 w-8 h-8 rounded-lg bg-red-500 hover:bg-red-600 text-white flex items-center justify-center shadow-lg"
      ...
      <motion.button
        ...
        className="hidden lg:flex absolute bottom-6 right-4 w-10 h-10 rounded-xl bg-gradient-to-br from-orange-500 to-orange-600 items-center justify-center shadow-2xl border-3 border-white ring-4 ring-orange-400/30 text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-orange-600"
  ...
  // Mobile: render in portal/fixed position when open
  if (isMobileOpen) {
    return (
